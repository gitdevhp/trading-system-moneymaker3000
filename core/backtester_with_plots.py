...
## THIS IS NOT ORIGINAL THIS IS MODIFIED BY JADEN FOR BETTER TESTING
# go to backtester.py for original file
@dataclass
class TradeRecord:
...
class Backtester:
    ...
    def run(self) -> pd.DataFrame:
        ...
        # Build market DataFrame from history and attach equity/cash/position
        market_df = pd.DataFrame(self.market_history).reset_index(drop=True)
        market_df = market_df.copy()
        market_df["equity"] = self.equity_curve
        market_df["cash"] = self.cash_history
        market_df["position"] = self.position_history
        return market_df
...
def plot_equity(equity_df: pd.DataFrame, save_path: Optional[Path] = None) -> None:
    """Plot equity curve and, if available, side-by-side price + position.

    Accepts the DataFrame returned by `Backtester.run()` which contains
    columns: `Datetime`, `Close`, `equity`, `cash`, `position` when available.
    """
    # If Close not present, fall back to single-panel equity plot
    has_price = "Close" in equity_df.columns
    ...
    # Two-panel side-by-side: equity on the left, price + position on the right
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5), gridspec_kw={"width_ratios": [1, 1.2]})

    # Equity panel
    ax1.plot(equity_df["equity"], label="Equity", color="tab:blue")
    ...
    # Price panel
    # Use index for x-axis if no Datetime index provided
    x = equity_df.index
    ax2.plot(x, equity_df["Close"], label="Close", color="black")

    # Shade long/short regions from `position` column
    if "position" in equity_df.columns:
        pos = equity_df["position"].fillna(0).astype(int)
        # Get min/max price for shading bounds
        price_min = equity_df["Close"].min()
        price_max = equity_df["Close"].max()
        # Long shading
        ax2.fill_between(x, price_min, price_max, where=(pos > 0), color="green", alpha=0.15,
                         label="Long Position")
        # Short shading
        ax2.fill_between(x, price_min, price_max, where=(pos < 0), color="red", alpha=0.15,
                         label="Short Position")

    ax2.set_title("Price with Position Shading")
    ax2.set_xlabel("Time")
