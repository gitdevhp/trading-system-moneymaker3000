"""
Offline backtest runner (enhanced plotting variant).

Usage:
    python run_backtest_with_plots.py --csv data\\AAPL_1Min_stock_alpaca_clean.csv --strategy ma --plot

This runner is identical to `run_backtest.py` but uses the enhanced plotting helper
from `core.backtester_with_plots` so you can keep the original runner untouched.
"""

##THIS IS NOT ORIGINAL FILE THIS IS MODIFIED BY JADEN FOR BETTER TESTING
# go to run_backtest.py for original file
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run an offline CSV backtest (enhanced plot variant).")
